...
var figlet = figlet || (function() {
    ...
    */
    ...
    function generateFigTextLines(txt, figChars, opts) {
        var charIndex, figChar, spaceIndex, overlap = 0, row, outputFigText, len, height=opts.height, outputFigLines = [], maxWidth, prevWidth, nextFigChars, prevFigChars, figWords = [], char, isSpace, testFigWord, textFigLine, tmpBreak;
            ...
            if (figChar) {
                ...
                if (opts.width > 0) {
                    if (opts.whitespaceBreak) {
                        ...
                        textFigLine = joinFigArray(figWords.concat(
                            nextFigChars, [{fig: figChar, overlap}]
                        ), height, opts);
                ...
                if (opts.width > 0 && opts.whitespaceBreak) {
                    if (isSpace) {
                        // break long words
                        do {
                            textFigLine = joinFigArray(nextFigChars, height, opts);
                            maxWidth = figLinesWidth(textFigLine);
                            if (maxWidth > opts.width) {
                                tmpBreak = breakWord(nextFigChars, height, opts);
                                nextFigChars = tmpBreak.chars;
                                outputFigLines.push(tmpBreak.outputFigText);
                            }
                        } while (maxWidth > opts.width);
                        // any leftovers
                        if (maxWidth > 0) {
                            figWords.push({fig: textFigLine, overlap});
                        }
                        // save space character and current overlap for smush in joinFigWords
                        figWords.push({fig: figChar, overlap: overlap, space: true});
                        prevFigChars = nextFigChars;
                        nextFigChars = [];
                        outputFigText = newFigChar(height);
                        continue;
                    } else {
                        nextFigChars.push({fig: figChar, overlap});
                    }
                }
        ...
        if (opts.width > 0 && opts.whitespaceBreak) {
            figWords.push({fig: joinFigArray(nextFigChars, height, opts), overlap});
