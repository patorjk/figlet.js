...
var figlet = figlet || (function() {
        ...
        */
        me.loadFont(fontName, function(err, fontOpts) {
            ...
            */
            ...
            next(null, generateText(fontName, myOpts, txt));
        });
    };
    ...
    */
    me.parseFont = function(fontName, data, next) {
        data = data.replace(/\r\n/g,"\n").replace(/\r/g,"\n");
        figFonts[fontName] = {};
        ...
        {
            delete figFonts[fontName];
            next && next('FIGlet header contains invalid values.');
            return;
        }
        ...
        */
        ...
        if (lines.length < (opts.numCommentLines + (opts.height * charNums.length)) ) {
            delete figFonts[fontName];
            next && next('FIGlet file is missing data.');
            return;
        }
    ...
    me.loadFont = function(fontName, next) {
        ...
        jQuery.ajax({
            ...
            success: function(data) {
                me.parseFont(fontName, data, function(err, options) {
                    next(err, options);
                });
            },
            error: function() {
