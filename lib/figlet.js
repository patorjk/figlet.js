...
var figlet = figlet || (function() {
    ...
    */
    ...
    function generateFigTextLines(txt, figChars, opts) {
        var charIndex, figChar, spaceIndex, overlap = 0, row, outputFigText, len, height=opts.height, outputFigLines = [], maxWidth, nextFigWord, figWords = [], char, isSpace, testFigWord, textFigLine;
        ...
        for (charIndex = 0; charIndex < len; charIndex++) {
            ...
            if (figChar) {
                ...
                if (opts.width > 0) {
                    // next character in last word
                    if (opts.whitespaceBreak) {
                        testFigWord = horizontalSmush(nextFigWord.fig, figChar, overlap, opts);

                        maxWidth = figLinesWidth(joinFigWords(figWords.concat({
                            fig: testFigWord,
                            overlap: nextFigWord.overlap
                        }), height, opts));
                    } else {
