...
var figlet = figlet || (function() {
    ...
    */
    ...
    /*
       join words into single Fig line
       - worList - array of ASCII words if form {fig: array, overlap: number}
       - len - height of the Characters
       - opt - options object
    */
    function joinFigWords(worList, len, opts) {
        return worList.reduce(function(acc, data) {
            return horizontalSmush(acc, data.fig, data.overlap, opts);
        }, newFigChar(len));
    }

    function generateFigTextLines(txt, figChars, opts) {
        var charIndex, figChar, spaceIndex, overlap = 0, row, outputFigText, len, height=opts.height, outputFigLines = [], maxWidth, nextFigWord, figWords = [], char, isSpace;

        outputFigText = newFigChar(height);
        if (opts.width > 0 && opts.whitespaceBreak) {
            nextFigWord = {
                fig: newFigChar(height),
                overlap: overlap
            };
        }
        ...
        for (charIndex = 0; charIndex < len; charIndex++) {
            ...
            if (figChar) {
                ...
                if (opts.width > 0) {
                    // word and space is added to word list
                    
                    maxWidth = figLinesWidth(joinFigWords(figWords.slice(0, -1), height, opts), figChar, outputFigText);

                    console.log(outputFigText.join('\n'))
                    console.log( joinFigWords(figWords.slice(0, -1), height, opts).join('\n') )
                    console.log(maxWidth);

                    if (maxWidth >= opts.width && charIndex > 0) {
                        console.log('width exceeded');
                        if (opts.whitespaceBreak) {
                            outputFigText = joinFigWords(figWords.slice(0, -1), height, opts);
                            console.log(outputFigText.join('\n'));
                            figWords = [];
                        }
                ...
                if (opts.width > 0 && opts.whitespaceBreak) {
                    if (isSpace) {
                        ...
                        nextFigWord.fig = horizontalSmush(nextFigWord.fig, figChar, overlap, opts);
                    }
                }
                outputFigText = horizontalSmush(outputFigText, figChar, overlap, opts);
            }
        }
        // add last line that was not broken
        if (opts.width > 0 && opts.whitespaceBreak) {
            figWords.push(nextFigWord);
            outputFigText = joinFigWords(figWords, height, opts);
        }
     ...
     */
    var figLinesWidth = function(textLines, charLines, additionalText) {
        
        if (additionalText) {
            return Math.max.apply(Math, textLines.map(function(line, i) {
                return line.length + charLines[i].length + additionalText[i].length;
            }));
        }

        if (charLines) {
            return Math.max.apply(Math, textLines.map(function(line, i) {
                return line.length + charLines[i].length;
            }));
        }
        return Math.max.apply(Math, textLines.map(function(line, i) {
            return line.length;
